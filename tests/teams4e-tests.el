...
(defun teams4e-test-forwarded-message ()
  "Return a synthetic native Teams forward with an attachment-only body."
  (let ((reference
         '((originalMessageContent
            . "<p>Forwarded <strong>detail</strong></p><p><a href=\"https://example.invalid/docs\">Docs</a></p>")
           (originalMessageSender . ((user . ((displayName . "Ada")))))
           (originalSentDateTime . "2026-09-22T10:00:00Z")
           (originalConversationId . "source-chat"))))
    (list '(id . "forward-1")
          '(from . ((user . ((displayName . "Grace")))))
          '(createdDateTime . "2026-09-22T11:00:00Z")
          '(body . ((contentType . "html")
                    (content . "<attachment id=\"forward-ref\"></attachment>")))
          (cons 'attachments
                (list (list '(id . "forward-ref")
                            '(contentType . "forwardedMessageReference")
                            (cons 'content (json-serialize reference))))))))

(ert-deftest teams4e-forwarded-message-renders-and-exports-original-content ()
  (let ((message (teams4e-test-forwarded-message))
        (teams4e-message-renderer 'plain)
        (teams4e--chats '(((id . "source-chat") (topic . "Design")))))
    (with-temp-buffer
      (teams4e--insert-message message)
      (should (string-match-p "Forwarded from Ada" (buffer-string)))
      (should (string-match-p "Forwarded detail" (buffer-string)))
      (should (string-match-p "2026-09-22" (buffer-string)))
      (should (string-match-p "Design" (buffer-string)))
      (should-not (string-match-p "Empty message\\|Attachment:" (buffer-string))))
    (let ((markdown (teams4e--message-markdown message)))
      (should (string-match-p "Forwarded from Ada" markdown))
      (should (string-match-p (regexp-quote "Forwarded **detail**") markdown))
      (should (string-match-p (regexp-quote "[Docs](https://example.invalid/docs)") markdown))
      (should-not (string-match-p "Empty message" markdown)))))

(ert-deftest teams4e-forwarded-message-rich-renderer-gets-forward-content ()
  (let (rendered)
    (cl-letf (((symbol-function 'teams4e--insert-rendered-markdown)
               (lambda (text) (push text rendered))))
      (teams4e--insert-message-reference (teams4e-test-forwarded-message) t))
    (should (string-match-p "Forwarded from Ada" (car rendered)))
    (should (string-match-p "https://example.invalid/docs" (car rendered)))))

(ert-deftest teams4e-forwarded-message-handles-multiple-and-malformed-references ()
  (let* ((message (teams4e-test-forwarded-message))
         (attachments (teams4e--get message 'attachments)))
    (setf (alist-get 'attachments message)
          (append attachments
                  '(((contentType . "messageReference")
                     (content . ((messagePreview . "Second reference")
                                 (messageSender . ((user . ((displayName . "Lin")))))))))
                  '(((contentType . "forwardedMessageReference") (content . "{broken")))))
    (let ((text (teams4e--message-reference-text message t)))
      (should (string-match-p "Forwarded detail" text))
      (should (string-match-p "Second reference" text))
      (should (string-match-p "Referenced content unavailable" text)))))

(ert-deftest teams4e-forwarded-message-copy-includes-original ()
  (with-temp-buffer
    (let ((teams4e-message-renderer 'plain)
          (kill-ring nil))
      (teams4e--insert-message (teams4e-test-forwarded-message))
      (goto-char (point-min))
      (teams4e-copy-message)
      (should (string-match-p "Forwarded detail" (car kill-ring))))))
          ...
          (save-current-buffer
            (cl-letf (((symbol-function 'pop-to-buffer)
                       (lambda (value &rest _)
                         (setq buffer value)
                         (set-buffer value)))
                      ((symbol-function 'teams4e-current-message) (lambda () source))
                      ((symbol-function 'teams4e--current-web-url)
                       (lambda () "https://example.invalid/source"))
                      ((symbol-function 'teams4e--select-chat)
                       (lambda (callback) (funcall callback target)))
                      ((symbol-function 'teams4e--run)
                       (lambda (&rest _) (ert-fail "Must not send before review"))))
              (teams4e--open-compose target)
              (insert "Existing draft")
              (setq teams4e-compose--content-type format)
              (teams4e-forward-message)
              (should (string-prefix-p "Existing draft" (buffer-string)))
              (should (string-match-p "Forwarded detail" (buffer-string)))
              (should (string-match-p "https://example.invalid/docs" (buffer-string)))
              (should (string-match-p "https://example.invalid/source" (buffer-string)))
              (should-not teams4e-compose--reply-to)
              (should (equal target teams4e-compose--target))
              (should (= #o600 (file-modes teams4e-compose--draft-file)))
...
(ert-deftest teams4e-forward-refuses-deleted-message ()
  (cl-letf (((symbol-function 'teams4e-current-message)
             (lambda () '((id . "gone") (deletedDateTime . "2026-09-22"))))
            ((symbol-function 'teams4e--current-web-url) #'ignore)
            ((symbol-function 'teams4e--select-chat)
             (lambda (_) (ert-fail "Must not select destination"))))
    (should-error (teams4e-forward-message) :type 'user-error)))

(ert-deftest teams4e-message-bounds-exclude-neighbors-and-separators ()
  (with-temp-buffer
    (let ((teams4e-message-renderer 'plain)
      ...
      (let ((start (point)))
        ...
        (let ((end (point)))
          (insert "Next day\n")
          (teams4e--insert-message
           '((id . "next") (body . ((content . "Other message")))))
          (dolist (pos (list start (+ start 3) (1- end)))
            (should (equal (cons start end) (teams4e-message-bounds pos))))
          (should-not (teams4e-message-bounds (point-min)))
          (should-not (teams4e-message-bounds end))
          (goto-char (+ start 3))
          (teams4e-mark-message)
          (should (= start (region-beginning)))
          (should (= end (region-end)))
          (let ((kill-ring nil))
            (call-interactively #'kill-ring-save)
            (should (string-match-p "Forwarded detail" (car kill-ring)))
            (should-not (string-match-p "Next day\\|Other message" (car kill-ring)))))))))

(ert-deftest teams4e-reader-copy-and-forward-bindings ()
  (should (eq #'teams4e-forward-message
              (lookup-key teams4e-action-map (kbd "f"))))
  (dolist (map (list teams4e-chat-mode-map teams4e-channel-thread-mode-map))
    (should (eq #'kill-ring-save (lookup-key map (kbd "M-w"))))
    (should (eq #'teams4e-mark-message (lookup-key map (kbd "M-h")))))
  (should (eq #'teams-unread-filter
              (lookup-key teams4e-recent-mode-map (kbd "F")))))
